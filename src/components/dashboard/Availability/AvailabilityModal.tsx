...
//       <div className="fixed inset-0 bg-opacity-50 bg-[rgba(0,0,0,0.5)] flex items-center justify-center p-4 z-50">
...
export const AvailabilityModal: React.FC<AvailabilityModalProps> = ({
  ...
  // Determine the current day (e.g., "thursday")
  const currentDayKey = dayjs().format("dddd").toLowerCase();
  ...
  const handleSave = async () => {
    ...
    try {
      const userData = localStorage.getItem("userData");
      if (!userData) throw new Error("Please log in again");

      const user = JSON.parse(userData);
      if (!user || !user._id) throw new Error("User information is missing");
      ...
      <div className="fixed inset-0 bg-opacity-50 bg-[rgba(0,0,0,0.5)] flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
          <div className="bg-gradient-to-r from-teal-600 to-teal-700 px-6 py-4 text-white">
            ...
            <p className="text-teal-100 mt-2">
              You can only set availability for <strong>{currentDayKey}</strong>
            </p>
          </div>
          ...
          <div className="p-6 max-h-[calc(90vh-200px)] overflow-y-auto">
            ...
            <div className="mb-6 opacity-50 pointer-events-none">
              ...
              <div className="flex flex-wrap gap-2">
                <button className="px-3 py-2 text-sm bg-teal-50 text-teal-700 rounded-lg">
                  Weekdays Only
                </button>
                <button className="px-3 py-2 text-sm bg-blue-50 text-blue-700 rounded-lg">
                  Full Week
                </button>
                <button className="px-3 py-2 text-sm bg-gray-50 text-gray-700 rounded-lg">
                  Clear All
                </button>
              </div>
            </div>
              ...
              {daysOfWeek.map(({ key, label }) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {schedule[key]?.isAvailable && key === currentDayKey && (
                      <div className="flex items-center text-xs text-teal-600 bg-teal-50 px-2 py-1 rounded-full">
                        <Clock className="w-3 h-3 mr-1" />
                        Available
                      </div>
